...
public class DLCTestCase extends MBIntegrationUiBaseTest {
     ...
     */
    @BeforeClass()
    public void createDLC() {
        Integer sendCount = 15;
        Integer runTime = 100;
        Integer expectedCount = 15;

        //Reduce AndesAckWaitTimeOut to minimize test duration
        System.setProperty("AndesAckWaitTimeOut", "5");

        AndesClient receivingClient = new AndesClient("receive", "127.0.0.1:5672", "queue:DLCTestQueue",
                                                      "5", "false", runTime.toString(), expectedCount.toString(),
                                                      "1", "listener=true,ackMode=2,delayBetweenMsg=0,ackAfterEach=215," +
                                                           "stopAfter=" + expectedCount + 200, "");
        receivingClient.startWorking();

        AndesClient sendingClient = new AndesClient("send", "127.0.0.1:5672", "queue:DLCTestQueue", "5", "false",
                                                    runTime.toString(), sendCount.toString(),
                                                    "1", "ackMode=2,delayBetweenMsg=0,stopAfter=" + sendCount, "");
        sendingClient.startWorking();

        //Thread sleep until messages sent to DLC after breaching maximum number of retrying
        AndesClientUtils.sleepForInterval(100000);
    }
