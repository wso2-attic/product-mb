...
do 
    ...
    data_file_path_inst="$DATAFILES$instant_sufix$filename"
    > $data_file_path
    > $data_file_path_inst
    ...
    > $data_recoder
    egrep "^summary =" $f > $data_recoder                   #Get the files begin with summary and creare file searched.txt
    > $data_recoder_inst
    egrep "^summary \+" $f > $data_recoder_inst
    #-------------------------------------
    ...
    do
        ...
        echo "$sample_length ${word_array[6]}" >> $data_file_path_inst      #Write graph data into file
        sample_length=$((sample_length+1))
    done <"$data_recoder_inst"
...
"$data_file_path_inst" with lines title "Instant_Average"
__EOF
done
