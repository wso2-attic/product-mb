...
INPUT_FILES=PATH-TO-INPUT-FILES-DIRECTORY/*          #Path for the directory where input text files containing message ID's are stored/*
DECRYPTED_ID_FILES=PATH-TO-DIRECTORY-WHERE-OUTPUT-SHOULD-BE-SAVED/   #Path for the directory where decrypted data files should be stored/
...
do 
    filename="${f##*/}"
    decrypted_data_path="$DECRYPTED_ID_FILES$filename"
    > $decrypted_data_path
    ...
    do
        ...
        then
            echo "INVALID MESSAGE ID" >> $decrypted_data_path 
        else
            ...
            echo "MESSAGE ID: $line DATE: $date_value NODE ID: $node_id_dec OFFSET: $offset_dec" >> $decrypted_data_path         #write output into file
        fi
    done <"$f"
