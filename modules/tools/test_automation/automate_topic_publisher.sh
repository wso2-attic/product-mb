  ...
  echo "Incorrect message tps size";
	...
	for i in $(seq $starting_message_size_index $last_num_messages_index); do 
		...
		for k in $(seq $starting_k $last_num_pub_index); do
			...
			num_messages_per_client=$(((($num_messages) / $num_pubs) / ${pubs[$k]}));
			...
			scenario="Topics message size:${message_size} nodes:${num_nodes} topics:${num_topics} publishers per topic:${pubs[$k]}";		
			logPub "$scenario"

			/usr/bin/expect << EOF
			spawn ./test_topic_publisher.sh
