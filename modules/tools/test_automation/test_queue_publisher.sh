...
publisher_resultfile_name=$queue_result_dir/$messageSize\_test_publisher_nodes_$nodes\_queues_$queues\_result.txt;
...
if [ "$is_mock_run" == "false" ]; then
	nohup $jmeterBinary -n -t $publisher_outfile_name > $publisher_resultfile_name &

	echo -e "\nRuning Jmeter ...\n"
	tail -f $publisher_resultfile_name
fi
