...
for i in $(seq $starting_message_size_index $last_num_messages_index); do 
	...
	if [ "$i" -eq "$starting_message_size_index" ]; then
		...
		for k in $(seq $starting_k $last_num_sub_index); do
			...
			scenario="Topics message size:$message_size nodes:$num_nodes topics:$num_topics subscribers per topic:${subs[$k]}";		
			logSub "$scenario"

			/usr/bin/expect << EOF
			spawn ./test_topic_subscriber.sh
...
EOF
			...
			sub_resultfile_name=$topic_result_dir/$message_size\_test_subscriber_nodes_$num_nodes\_topics_$num_topics\_${subs[$k]}\_result.txt;
